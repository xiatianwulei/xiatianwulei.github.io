...
Swift 6 对于从 5.5 迁移的项目而言，最核心的挑战在于**线程安全模型的重构**，但除此之外，语言语法、标准库、工具链等方面的变化同样需要重点关注。
...
*   **适配策略**：
    ...
    *   使用 `MainActor.run` 包裹后台线程的 UI 操作：
    ...
    // 正确示例 但当需要更新 UI 时，使用DispatchQueue.main.async将 UI 操作切换到主线程执行
    DispatchQueue.global().async {
       let data = fetchData()
        // 使用主队列异步调度，切换到主线程更新UI
...
// 被 @frozen 标记后，未来不能再添加新成员（如 case connectionFailed）

// 用 actor 定义 Actor 类型
actor SafeActor { // 这里 actor 修饰的是整个类型
    var value: Int = 0 // Actor 的属性
    func increment() { value += 1 } // Actor 的方法
}
...
// 1. 定义一个 Actor 类型（天然符合 Sendable，线程安全）
actor UserProfile {
    var age: Int = 0
    func updateAge(to newAge: Int) { age = newAge }
}

// 2. 结构体包含 Actor 实例（引用类型），但仍可声明为 Sendable
struct User: Sendable {
    let id: Int
    let name: String
    let profile: UserProfile // Actor 是 Sendable 类型，安全跨线程传递
}
```
...
*   对无法满足 `Sendable` 的类型，使用 `@preconcurrency` 临时兼容（需谨慎）：
    ...
    // 包含非 Sendable 成员的类（普通类，未保证线程安全）
    class LegacyData {
       var value: Int = 0 // 可能被多线程修改，不符合 Sendable
    }

    // 用 @preconcurrency 标记结构体，暂时绕过 Sendable 检查
    @preconcurrency
    struct LegacyConfig {
       let data: LegacyData // LegacyData 不是 Sendable
    }

    // 在跨线程场景中使用（Swift 6 中原本会报错，用 @preconcurrency 后允许）
    Task {
      let config = LegacyConfig(data: LegacyData())
     await someAsyncFunction(config) // 暂时不报错
    }
    ```
...
*   `async/await`** 升级**：Swift 6 完善了异步函数的错误处理与取消机制，支持 `withTaskCancellationHandler` 优雅处理任务取消。
    ...
    ```
     func loadData() async {
         //  withTaskCancellationHandler的正确用法
            //    - operation: 实际的操作代码
            //    - onCancel: 取消时的处理逻辑
            //    Swift 6中参数顺序调整为operation在前，更符合逻辑
            await withTaskCancellationHandler(operation: { [weak self] in
              ...
              // 5. 取消回调中调用取消方法
              //    - 注意需要用await，因为cancelRequest是@MainActor方法
              //    - 这里会触发isCancelled状态更新
             Task {
...
*   **解决方案**：
    ...
    *   优先升级至支持 Swift 6 的库版本（如 RxSwift 6.6+）。
